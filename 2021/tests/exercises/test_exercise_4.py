...
def test_4a():
    assert exercise_4.run_a(TEST_DATA) == 4512


def test_returns_correct_answer_on_exercise_a():
    input_data = utils.load_data(exercise_4.EXERCISE)
    assert exercise_4.run_a(input_data) == 2496


def test_4b():
    assert exercise_4.run_b(TEST_DATA) == 1924


def test_returns_correct_answer_on_exercise_b():
    input_data = utils.load_data(exercise_4.EXERCISE)
    assert exercise_4.run_b(input_data) == 25925
